...
async function syncDatabase() {
  try {
    ...
    const defaults = [
      { key: 'appName', value: 'LabInfo LIMS' },
      { key: 'appLogo', value: '' },
      { key: 'loginBackground', value: 'https://images.unsplash.com/photo-1581093588401-fbb62a02f120?auto=format&fit=crop&q=80&w=2070' },
      { key: 'themeColor', value: '#2563eb' },
      { key: 'loginTitle', value: 'LabInfo LIMS' },
      { key: 'loginSubtitle', value: 'Laboratory Information Management System. Please sign in to access the system.' },
...
async function startServer() {
      ...
      if (!user) {
        ...
        if (user) {
          ...
          user = await User.create({ 
            ...
            email, 
            displayName, 
            role: role || (email === 'medialabindonesia@gmail.com' ? 'admin' : 'analyst'),
  ...
  app.post('/api/requisitions/:id/receive', async (req, res) => {
    try {
      ...
      if (labItem) {
        await labItem.increment('quantity', { by: requisition.get('quantity') as number });
      } else {
        await StockItem.create({
          ...
          quantity: requisition.get('quantity'),
  ...
  app.post('/api/daily-use', async (req, res) => {
    try {
      ...
      if (!labItem || (labItem.get('quantity') as number) < quantity) {
        return res.status(400).json({ error: 'Insufficient lab stock' });
      }

      await labItem.decrement('quantity', { by: quantity });
